...
-->
   ...
   <xsl:template match="/" mode="xmlverb">
      ...
      <fo:block white-space-collapse="false" line-feed-treatment="preserve" white-space-treatment="preserve" text-align="left"> 
      <!--  class="xmlverb-default"> -->
   ...
   <xsl:template match="*" mode="xmlverb">
      ...
      <xsl:if test="$ns-prefix != ''">
         <fo:inline> <!--class="xmlverb-element-nsprefix"> -->
            <xsl:value-of select="$ns-prefix"/>
         </fo:inline>
         <xsl:text>:</xsl:text>
      </xsl:if>
      <fo:inline> <!--  class="xmlverb-element-name"> -->
         <xsl:value-of select="local-name()"/>
      </fo:inline>
      <xsl:variable name="pns" select="../namespace::*"/>
      <xsl:if test="$pns[name()=''] and not(namespace::*[name()=''])">
         <fo:inline> <!--class="xmlverb-ns-name"> -->
   ...
   <xsl:template name="xmlverb-attrs">
      <xsl:text> </xsl:text>
      <fo:inline> <!--class="xmlverb-attr-name"> -->
         <xsl:value-of select="name()"/>
      </fo:inline>
      <xsl:text>="</xsl:text>
      <fo:inline> <!-- class="xmlverb-attr-content"> -->
   ...
   <xsl:template name="xmlverb-ns">
      <xsl:if test="name()!='xml'">
         ...
         <xsl:text>="</xsl:text>
         <fo:inline> <!--class="xmlverb-ns-uri"> -->
            <xsl:value-of select="."/>
         </fo:inline>
         <xsl:text>"</xsl:text>
      </xsl:if>
   ...
   <xsl:template match="comment()" mode="xmlverb">
      <xsl:text>&lt;!--</xsl:text>
      <fo:inline> <!--class="xmlverb-comment"> -->
         <xsl:call-template name="preformatted-output">
            <xsl:with-param name="text" select="."/>
         </xsl:call-template>
      </fo:inline>
   ...
   <xsl:template match="processing-instruction()" mode="xmlverb">
      <xsl:text>&lt;?</xsl:text>
      <fo:inline> <!--class="xmlverb-pi-name"> -->
         <xsl:value-of select="name()"/>
      </fo:inline>
      <xsl:if test=".!=''">
         <xsl:text> </xsl:text>
         <fo:inline> <!--class="xmlverb-pi-content"> -->
            <xsl:value-of select="."/>
         </fo:inline>
      </xsl:if>
      <xsl:text>?&gt;</xsl:text>
