...
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:verb="http://informatik.hu-berlin.de/xmlverbatim" exclude-result-prefixes="verb">
...
-->
   ...
   <!-- element nodes -->
   <xsl:template match="*" mode="xmlverb">
      <xsl:param name="mains" select="'dflt'"/>
      <xsl:param name="indent-elements" select="false()"/>
      <xsl:param name="indent" select="''"/>
      <xsl:param name="indent-increment" select="'   '"/>
      <xsl:param name="owncall" select="0"/>
      
        <xsl:variable name="locnm">-<xsl:value-of select="local-name()"/>-</xsl:variable>
       <xsl:if test="contains($mains, $locnm)"><br/></xsl:if>
       
      
      <xsl:if test="$indent-elements">
         <!--  before a new element -->
        <xsl:if test="$owncall = 1">
            <br/>
           </xsl:if>
      ...
      <xsl:choose>
         <xsl:when test="node()">
            <xsl:text>&gt;</xsl:text>
            <xsl:apply-templates mode="xmlverb">
                <xsl:with-param name="mains" select="$mains"/>
              <xsl:with-param name="indent-elements" select="$indent-elements"/>
              <xsl:with-param name="indent" select="concat($indent, $indent-increment)"/>
              <xsl:with-param name="indent-increment" select="$indent-increment"/>
              <xsl:with-param name="owncall" select="1"/>
            </xsl:apply-templates>
            
            <!--  was test="* and $indent-elements" -->
            <xsl:if test="$indent-elements">
               <br/>
               <!--  end of last child -->
               <xsl:value-of select="$indent"/>
   ...
   <xsl:template match="comment()" mode="xmlverb">
      ...
      <xsl:text>--&gt;</xsl:text> 
      <xsl:if test="not(parent::*)">ff<br/><xsl:text>
</xsl:text></xsl:if>
   </xsl:template>
   ...
   <xsl:template match="processing-instruction()" mode="xmlverb">
      ...
      <xsl:text>?&gt;</xsl:text>
      <xsl:if test="not(parent::*)">gg<br/><xsl:text>
</xsl:text></xsl:if>
   </xsl:template>
